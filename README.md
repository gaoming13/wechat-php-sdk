...
## 功能模块
Wechat （处理自动接入、获取与回复微信消息）([使用说明](#wechat))
* [接收普通消息/事件推送](#wechat-1)
* [被动回复（文本、图片、语音、视频、音乐、图文）](#wechat-2)
* [转发到多客服接口](#wechat-3)

Api （处理需要access_token的主动接口）([使用说明](#api))
* [主送发送客服消息（文本、图片、语音、视频、音乐、图文）](#api-1)
* [多客服功能（客服管理、多客服回话控制、获取客服聊天记录等）](#api-2)
* [素材管理（临时素材、永久素材、素材统计）](#api-3)
* [自定义菜单管理（创建、查询、删除菜单）](#api-4)
* [微信JSSDK（生成微信JSSDK所需的配置信息）](#api-5)
* [账号管理（生成带参数的二维码、长链接转短链接接口）](#api-6)
* [用户管理（用户分组管理、设置用户备注名、获取用户基本信息、获取用户列表、网页授权获取用户基本信息）](#api-7)
* [微信公众号支付(JSAPI)](#api-8)
* [微信App支付(App)](#api-9)
* 数据统计接口（开发中...）
